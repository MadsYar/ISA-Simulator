...
# Creating a variable to repeat the simulation
if (max_cycles > 0):
    simulation = True

# Starting the simulation which is true until max cycles are reached
while(simulation):
    
    # The next three lines of codes sets the program counter, OPCODES and the OPERANDS
    PC = program_counter
    OPCODE = instructionMemory.read_opcode(PC)
    OPERANDS = ", " + instructionMemory.read_operand_1(PC) + ", " + instructionMemory.read_operand_2(PC) + ", " + instructionMemory.read_operand_3(PC)
    
    # The next three lines of codes print the cycle number, program counter and the current instruction
    print("Current cycle = " + str(current_cycle))
    print("Program counter = " + str(program_counter))
    print("Instruction = " + OPCODE + OPERANDS.replace(", -", ""))  
    
    # Addition
    if (OPCODE == "ADD"):
        LOAD = instructionMemory.read_operand_1(PC)
        NUMBER_1 = registerFile.read_register(instructionMemory.read_operand_2(PC))
        NUMBER_2 = registerFile.read_register(instructionMemory.read_operand_3(PC))
        
        print(str(instructionMemory.read_operand_2(PC)) + " = " + str(NUMBER_1) + " and " + str(instructionMemory.read_operand_3(PC)) + " = " + str(NUMBER_2))
        
        registerFile.write_register(LOAD, (NUMBER_1 + NUMBER_2))
        print("The new value is " + str(registerFile.read_register(instructionMemory.read_operand_2(PC))))
    ...
    # Or function
        ...
        LOAD = instructionMemory.read_operand_1(PC)
        NUMBER1 = registerFile.read_register(instructionMemory.read_operand_2(PC))
        NUMBER2 = registerFile.read_register(instructionMemory.read_operand_3(PC))
        
        registerFile.write_register(LOAD, (NUMBER1 | NUMBER2))
    ...
    # Not function
        ...
        LOAD = instructionMemory.read_operand_1(PC)
        NUMB = registerFile.read_register(instructionMemory.read_operand_2(PC))
        
        registerFile.write_register(LOAD, ~NUMB)
    
    # Load immediate
    elif (OPCODE == "LI"):
        LOAD = instructionMemory.read_operand_1(PC)
        NUMB = instructionMemory.read_operand_2(PC)
        
        print(LOAD + " = " + NUMB)
        registerFile.write_register(LOAD, int(NUMB))
    
    # Load data
    elif (OPCODE == "LD"):
        LOAD = instructionMemory.read_operand_1(PC)
        DATA = registerFile.read_register(instructionMemory.read_operand_2(PC))
        
        registerFile.write_register(LOAD, dataMemory.read_data(DATA))
        print(str(LOAD) + " = " + str(DATA))
      
    # Store data    
    elif (OPCODE == "SD"):
        NUMB = registerFile.read_register(instructionMemory.read_operand_1(PC))
        DATA = registerFile.read_register(instructionMemory.read_operand_2(PC))
        
        dataMemory.write_data(DATA, NUMB)
    
    # Jump
    elif (OPCODE == "JR"):
        NUMB = registerFile.read_register(instructionMemory.read_operand_1(PC))
        program_counter = NUMB
        
        print("We can now go R1 which have the instruction = " + str(NUMB))
    ...
    # Jump if equal
        ...
        print(str(instructionMemory.read_operand_2(PC)) + " = " + str(NUMBER1) + " | " + str(instructionMemory.read_operand_3(PC)) + " = " + str(NUMBER2))
    
        if (NUMBER1 == NUMBER2):
            print(str(NUMBER1) + " and " + str(NUMBER2) + " are the same. Therefore, it is needed to jump to the instruction = " + str(NUMB))
            program_counter = NUMB
        else:
            print(str(NUMBER1) + " and " + str(NUMBER2) + " are not the same. Therefore, it is needed to pass")
            program_counter += 1
    ...
    # Jump if less than
        ...
        if (NUMBER1 < NUMBER2):
            print("Because " + str(NUMBER1) + " is smaller than " + str(NUMBER2) + ", it is needed to jump to the instruction = " + str(NUMB))
            program_counter = NUMB
        else:
            program_counter += 1
    ...
    # No operation
        ...
        print("Unfortunately, it is needed to pass")
        pass
    ...
    if (OPCODE != "JR" and OPCODE != "JEQ" and OPCODE != "JLT"):
        program_counter += 1
    ...
    if (current_cycle == max_cycles):
        print("This is the end - max cycled have now been reached")
        simulation = False
